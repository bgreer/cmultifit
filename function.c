...
int funk_corr (int m, int n, double* p, double *deviates, double **derivs, void *private)
{
	struct kslice *sub;
	int istw, iendw, num, iw, itht, ik, ii;
	double akt, w1, tht, den, x2, err, back, back2, cost, sint;
	...
	for (ik=0; ik<(n-NBACK)/NPEAK; ik++)
	{
		...
		for (iw=istw; iw<=iendw; iw++)
		{
			...
			{
				den = w1 + thtarr[itht];
				den = den*den + 0.25*p[ik*NPEAK+2]*p[ik*NPEAK+2];
				x2 = thtpow[itht]/den;
				rdeviates[num] += (p[ik*NPEAK+1]*(w1+thtarr[itht]) + p[ik*NPEAK+7]*p[ik*NPEAK+2]/2.)/den;
				ideviates[num] += (p[ik*NPEAK+7]*(w1+thtarr[itht]) - p[ik*NPEAK+1]*p[ik*NPEAK+2]/2.)/den;
				num++;
			}
		}
	}

	/* Second step: combine real and imaginary parts, square */
	num = 0;
	for (iw=istw; iw<=iendw; iw++)
	{
		for (itht=0; itht<sub->ntheta; itht++)
		{
			deviates[num] = deviates[num]+(rdeviates[num]*rdeviates[num] + ideviates[num]*ideviates[num]);
			num++;
		}
	}

	/* Third step: subtract from data, weight appropriately */
	num = 0;
	for (iw=istw; iw<=iendw; iw++)
	{
		for (itht=0; itht<sub->ntheta; itht++)
